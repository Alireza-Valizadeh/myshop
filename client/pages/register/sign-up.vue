<template>
  <v-form @submit.prevent="submitForm" ref="createUserForm">
    <v-container>
      <FormText
        @input="lazyCaller(() => updateCreateUserArr({ k: 'name', v: $event }))"
        :value="newUser.name"
        :rules="[mustFillRule]"
      ...
      <FormText
        ...
        @input="
          lazyCaller(() => updateCreateUserArr({ k: 'password', v: $event }))
        "
        :append-icon="
          showPassword
      ...
      <FormText
        ...
        @input="
          lazyCaller(() =>
            updateCreateUserArr({ k: 'confirmPassword', v: $event })
          )
        "
        :append-icon="
          showConfirmPassword
...
export default {
  ...
  methods: {
    ...
    emailFormatRule,
    passwordFormatRule,
    // TODO : fix matchpassword function
    matchPassword(pass, retypePass) {
      if (!retypePass || retypePass.length < 8 || retypePass !== pass)
        return 'رمزعبور وارد شده با تکرار آن برابر نمیباشد!';
      else return true;
    },
